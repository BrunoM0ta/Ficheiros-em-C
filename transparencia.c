...
void transparencia(FILE *f1,FILE *f2,FILE *f3,int alfa,int x,int y){
...
/*  for (int j =y,m=0; j<image1.height && m<image.height; j++,m++){ // aqui é preciso inserir a primeira imagem(no enunciado, clock.ppm), a partir da posição x e y da segunda imagem, que vai percorrer as restantes lnihas desta última
   for (int k = x,n=0; k < image1.width && n<image.width; k++,n++){ // a mesma coisa para as colunas
     resultado.mat[j][k].r= alfa*image.mat[j][k].r +  (1-alfa)*image1.mat[j][k].r; // aqui é necessário que os índices da primeira imagem partam da origem
     resultado.mat[j][k].g= alfa*image.mat[j][k].g+ (1-alfa)*image1.mat[j][k].g;
     resultado.mat[j][k].b= alfa*image.mat[j][k].b + (1-alfa)*image1.mat[j][k].b;
  }
}*/
  ...
  alfa=atof(argv[1]);
   ...
   if(argc > 5){
   f2 = fopen(argv[5], "r"); // lê a segunda imagem
  ...
  if(f2==NULL){
   ...
   if(argc > 6){
   f3 = fopen(argv[6], "w"); // escreve no ficheiro de saída
   }
   else{
   f3 = stdout;
   }

  transparencia(f1,f2,f3,alfa,x,y);
   return 0;
  }
